...
if "questions" not in st.session_state:
    st.session_state.update({
        "questions": [], "current_idx": 0, "answers": {}, "done": False
    })
...
unit_name = st.sidebar.text_input("Unit Name/Number", placeholder="e.g., Unit 1")
...
if st.sidebar.button("🚀 Generate Exam"):
    if not unit_name:
        st.sidebar.error("Enter a Unit name!")
    else:
        book_ref = process_book(grade, sub)
        if book_ref:
            # CHANGED: Using gemini-2.0-flash (The current stable model)
            model = genai.GenerativeModel("gemini-2.0-flash") 
            prompt = f"Find '{unit_name}' in this book. Generate 20 MCQs. Format: JSON array ONLY. Keys: q, a, b, c, d, ans, exp."
